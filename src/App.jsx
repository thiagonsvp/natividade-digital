...
const activeMRR = c => {
  if(!c.valueSchedule?.length) return Number(c.value||0);
  const t=today(), a=c.valueSchedule.filter(p=>p.from<=t).sort((a,b)=>b.from.localeCompare(a.from));
  return a.length?Number(a[0].value):Number(c.valueSchedule[0]?.value||0);
};
...
const D_PLANS   = ["Starter","Pro","Enterprise","Personalizado"];
const D_STAGES  = ["Prospecção","Proposta Enviada","Negociação","Fechado"];
const D_ATT     = ["Reunião","E-mail","Ligação","WhatsApp","Visita"];
const D_USERS   = [{id:"u1",name:"Admin",email:"admin@natividadedigital.com",password:"admin123",role:"admin",permissions:[...ALL_PAGES]}];
const D_CLIENTS = [
  {id:"c1",name:"Clínica Bella Forma",plan:"Pro",status:"Ativo",since:"2024-01-15",contractStart:"2025-01-01",contractEnd:"2026-01-01",valueSchedule:[{from:"2025-01-01",value:"2000",tipo:"recorrente",parcelas:""},{from:"2025-06-01",value:"2500",tipo:"recorrente",parcelas:""}],recurringDay:"5",tags:["Saúde","Meta Ads"],contact:"ana@bellaforma.com",phone:"(11) 9999-1111"},
  {id:"c2",name:"Auto Peças Silva",plan:"Starter",status:"Ativo",since:"2024-03-01",contractStart:"2024-03-01",contractEnd:"2025-03-01",valueSchedule:[{from:"2024-03-01",value:"1200",tipo:"recorrente",parcelas:""}],recurringDay:"10",tags:["Varejo"],contact:"carlos@autopecas.com",phone:"(11) 9999-2222"},
];
const D_KANBAN  = {"Prospecção":[{id:"k1",name:"Academia Fitness",value:1800,resp:"Lucas"}],"Proposta Enviada":[{id:"k2",name:"E-commerce ModaVip",value:4200,resp:"Lucas"}],"Negociação":[{id:"k3",name:"OdontoTop",value:2800,resp:"Ana"}],"Fechado":[]};
const D_HISTORY = [{id:"h1",client:"Clínica Bella Forma",type:"Reunião",date:"2025-03-05",time:"14:00",user:"Ana",note:"Revisão de relatório. ROAS 4.2x."}];
const D_TASKS   = [{id:"t1",title:"Criativos Páscoa",client:"Clínica Bella Forma",assignee:"Lucas",due:"2025-04-10",priority:"Alta",done:false},{id:"t2",title:"Relatório mensal",client:"Clínica Bella Forma",assignee:"Pedro",due:"2025-04-12",priority:"Média",done:true}];
const D_REC     = [{id:"r1",client:"Clínica Bella Forma",value:2500,status:"Pago",due:"2025-03-05",paid:"2025-03-05",description:"Mensalidade Mar/25"},{id:"r2",client:"Auto Peças Silva",value:1200,status:"Pago",due:"2025-03-10",paid:"2025-03-10",description:"Mensalidade Mar/25"},{id:"r3",client:"Clínica Bella Forma",value:2500,status:"Pendente",due:"2025-04-05",paid:"",description:"Mensalidade Abr/25"}];
const D_PAY     = [{id:"p1",description:"Servidor Cloud",supplier:"AWS",value:450,category:"Infra",due:"2025-04-15",paid:"",status:"Pendente"},{id:"p2",description:"Licença Adobe",supplier:"Adobe",value:280,category:"Ferramentas",due:"2025-04-20",paid:"",status:"Pendente"}];
...
function FRow({label,children,C,half}){return <div style={{marginBottom:11,gridColumn:half?"span 1":"auto"}}><label style={{fontSize:10,color:C.muted,display:"block",marginBottom:3,textTransform:"uppercase",letterSpacing:.7,fontWeight:700}}>{label}</label>{children}</div>;}
  ...
  return <div style={{position:"fixed",inset:0,background:"#00000090",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:12}}>
    <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:16,padding:24,width:360,boxShadow:C.shadow}}>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
        <Btn C={C} ghost onClick={onNo}>Cancelar</Btn>
        <Btn C={C} danger onClick={onYes}>Excluir</Btn>
      </div>
    </div>
  </div>;
}
...
function ListEditor({title,items,setItems,C,onClose}){
  ...
  return <Modal C={C} title={`Editar lista: ${title}`} onClose={onClose}>
    <div style={{display:"flex",gap:7,marginBottom:12}}><input value={draft} onChange={e=>setDraft(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&draft.trim()){setItems(p=>[...p,draft.trim()]);setDraft("");}}} placeholder="Novo item..." style={{...iS(C),flex:1}}/><Btn C={C} sm onClick={()=>{if(!draft.trim())return;setItems(p=>[...p,draft.trim()]);setDraft("");}}>Add</Btn></div>
    {items.map((it,i)=><div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 11px",background:C.bg,borderRadius:8,marginBottom:6}}>
      ...
      <button onClick={()=>setItems(p=>p.filter((_,j)=>j!==i))} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:14}}>✕</button>
    </div>)}
...
// ── Login Page ────────────────────────────────────────────
function Login({users,onLogin,theme,toggleTheme}){
  const C=TH[theme];
  ...
  return <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"DM Sans,sans-serif"}}>
    <div style={{width:420,padding:40,background:C.surface,border:`1px solid ${C.border}`,borderRadius:24,boxShadow:C.shadow}}>
      <div style={{textAlign:"center",marginBottom:32}}>
        ...
        <div style={{color:C.muted,fontSize:13,marginTop:10}}>Faça login para acessar o sistema</div>
      </div>
      ...
      <div style={{marginBottom:14}}>
        <label style={{fontSize:11,color:C.muted,display:"block",marginBottom:4,textTransform:"uppercase",letterSpacing:.7,fontWeight:700}}>E-mail</label>
        <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="email@empresa.com" type="email" style={iS(C)} onKeyDown={e=>e.key==="Enter"&&attempt()}/>
      </div>
      <div style={{marginBottom:20}}>
        <label style={{fontSize:11,color:C.muted,display:"block",marginBottom:4,textTransform:"uppercase",letterSpacing:.7,fontWeight:700}}>Senha</label>
        <div style={{position:"relative"}}>
          <input value={pass} onChange={e=>setPass(e.target.value)} placeholder="••••••••" type={show?"text":"password"} style={iS(C)} onKeyDown={e=>e.key==="Enter"&&attempt()}/>
          <button onClick={()=>setShow(s=>!s)} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:13}}>{show?"🙈":"👁"}</button>
        </div>
      </div>
      <button onClick={attempt} style={{width:"100%",background:C.accent,color:"#fff",border:"none",borderRadius:10,padding:"12px",fontWeight:700,cursor:"pointer",fontSize:15,fontFamily:"Syne",letterSpacing:.3}}>Entrar</button>
      <div style={{textAlign:"center",marginTop:20}}>
        <button onClick={toggleTheme} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:12,fontFamily:"DM Sans"}}>{theme==="dark"?"☀️ Modo Claro":"🌙 Modo Escuro"}</button>
      </div>
      <div style={{marginTop:24,padding:"12px 16px",background:C.bg,borderRadius:10,fontSize:11,color:C.muted}}>
        <strong style={{color:C.text}}>Demo:</strong> admin@natividadedigital.com / admin123
      </div>
    </div>
  </div>;
}
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:20,boxShadow:C.shadow}}>
        ...
        {history.slice(0,4).map(h=><div key={h.id} style={{padding:"9px 0",borderBottom:`1px solid ${C.border}`}}>
          ...
          <div style={{fontSize:11,color:C.muted}}>{h.type} · {h.user} — {h.note?.slice(0,55)}...</div>
        </div>)}
...
// ── Clientes ──────────────────────────────────────────────
const EMPTY_C=()=>({name:"",plan:"",status:"Ativo",since:"",contractStart:"",contractEnd:"",valueSchedule:[{from:"",value:"",tipo:"recorrente",parcelas:""}],recurringDay:"",tags:"",contact:"",phone:""});
function Clientes({clients,setClients,history,plans,setPlans,setRec,C}){
  ...
  const updVS=(i,fk,v)=>ff({valueSchedule:form.valueSchedule.map((p,j)=>j===i?{...p,[fk]:v}:p)});

  function openNew(){setForm({...EMPTY_C(),plan:plans[0]||""});setModal("new");}
  function openEdit(c){setForm({...c,tags:(c.tags||[]).join(", "),valueSchedule:c.valueSchedule?.length?c.valueSchedule:[{from:"",value:"",tipo:"recorrente",parcelas:""}]});setModal("edit");}

  function generateInstallments(clientName, vs){
    const rows=[];
    vs.filter(p=>p.from&&p.value).forEach(p=>{
      const val=Number(p.value);
      if(p.tipo==="parcelado"&&p.parcelas>0){
        for(let i=0;i<Number(p.parcelas);i++){
          const due=addMonths(p.from,i);
          rows.push({id:uid(),client:clientName,value:val,status:"Pendente",due,paid:"",description:`Parcela ${i+1}/${p.parcelas}`});
        }
      } else {
        // recorrente — gera 12 meses
        for(let i=0;i<12;i++){
          const due=addMonths(p.from,i);
          rows.push({id:uid(),client:clientName,value:val,status:"Pendente",due,paid:"",description:`Mensalidade ${new Date(due+"T12:00:00").toLocaleDateString("pt-BR",{month:"short",year:"numeric"})}`});
        }
      }
    });
    return rows;
  ...
  function save(){
    ...
    const vs=form.valueSchedule.filter(p=>p.from&&p.value);
    const obj={...form,id:modal==="new"?uid():form.id,tags,valueSchedule:vs};
    if(modal==="new"){
      setClients(p=>[...p,obj]);
      const rows=generateInstallments(form.name,vs);
      if(rows.length) setRec(p=>[...p,...rows]);
    } else {
  ...
  if(sel) return <div>
    <button onClick={()=>setSel(null)} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",marginBottom:16,fontSize:13,display:"flex",alignItems:"center",gap:5}}>← Voltar para Clientes</button>
    <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:18}}>
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:22,boxShadow:C.shadow}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[["Plano",sel.plan],["MRR Atual",`R$ ${activeMRR(sel).toLocaleString()}`],["Contrato",`${fmt(sel.contractStart)} – ${fmt(sel.contractEnd)}`],["Contato",sel.contact],["Telefone",sel.phone],["Cliente desde",fmt(sel.since)],["Venc. Recorrente",sel.recurringDay?`Dia ${sel.recurringDay} de cada mês`:"—"]].map(([k,v])=>
        ...
        {sel.valueSchedule?.length>0&&<div style={{marginTop:12,padding:"11px 13px",background:C.bg,borderRadius:9}}>
          <div style={{fontSize:10,color:C.muted,textTransform:"uppercase",letterSpacing:.7,marginBottom:8}}>Escalonamento de Valor</div>
          {sel.valueSchedule.map((p,i)=><div key={i} style={{fontSize:13,color:C.text,marginBottom:4}}>{fmt(p.from)} → <strong style={{color:C.accent}}>R$ {Number(p.value).toLocaleString()}</strong> <span style={{fontSize:11,color:C.muted}}>({p.tipo==="parcelado"?`${p.parcelas}x`:"recorrente"})</span></div>)}
        </div>}
        <div style={{marginTop:14}}><Btn C={C} sm ghost danger onClick={()=>setConfirm(sel.id)}>🗑 Excluir Cliente</Btn></div>
      </div>
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:22,boxShadow:C.shadow}}>
    ...
    {confirm&&<Confirm C={C} msg="Deseja realmente excluir este cliente?" onNo={()=>setConfirm(null)} onYes={()=>{setClients(p=>p.filter(c=>c.id!==confirm));setSel(null);setConfirm(null);}}/>}
  </div>;
  ...
  return <div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      ...
      {!list.length&&<div style={{color:C.muted,textAlign:"center",padding:40,fontSize:14}}>Nenhum cliente encontrado.</div>}
    </div>
    {modal&&<ClienteFormModal C={C} form={form} ff={ff} updVS={updVS} plans={plans} modal={modal} onClose={()=>setModal(null)} onSave={save}/>}
    {planEd&&<ListEditor C={C} title="Planos" items={plans} setItems={setPlans} onClose={()=>setPlanEd(false)}/>}
  </div>;
}
...
function ClienteFormModal({C,form,ff,updVS,plans,modal,onClose,onSave}){
  return <Modal C={C} title={modal==="new"?"Novo Cliente":"Editar Cliente"} onClose={onClose} xlarge>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      ...
      <FRow C={C} label="Dia Venc. Recorrente"><Inp C={C} type="number" value={form.recurringDay} onChange={v=>ff({recurringDay:v})} placeholder="5" min="1" max="31"/></FRow>
      <FRow C={C} label="Início do Contrato"><Inp C={C} type="date" value={form.contractStart} onChange={v=>ff({contractStart:v})}/></FRow>
      <FRow C={C} label="Fim do Contrato"><Inp C={C} type="date" value={form.contractEnd} onChange={v=>ff({contractEnd:v})}/></FRow>
      <FRow C={C} label="Tags (separar por vírgula)"><Inp C={C} value={form.tags} onChange={v=>ff({tags:v})} placeholder="Saúde, Meta Ads"/></FRow>
      <FRow C={C} label="E-mail"><Inp C={C} value={form.contact} onChange={v=>ff({contact:v})} placeholder="email@empresa.com"/></FRow>
      <FRow C={C} label="Telefone"><Inp C={C} value={form.phone} onChange={v=>ff({phone:v})} placeholder="(11) 99999-0000"/></FRow>
    </div>
    <div style={{margin:"12px 0",padding:12,background:C.bg,borderRadius:10,border:`1px solid ${C.border}`}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        <span style={{fontSize:11,color:C.muted,textTransform:"uppercase",letterSpacing:.7,fontWeight:700}}>Escalonamento de Valor (MRR / Parcelas)</span>
        <Btn C={C} sm ghost onClick={()=>ff({valueSchedule:[...form.valueSchedule,{from:"",value:"",tipo:"recorrente",parcelas:""}]})}>+ Período</Btn>
      </div>
      {form.valueSchedule.map((p,i)=><div key={i} style={{display:"grid",gridTemplateColumns:"1.2fr 1fr 1.2fr 0.8fr auto",gap:8,alignItems:"end",marginBottom:10,padding:"10px",background:C.surface,borderRadius:8,border:`1px solid ${C.border}`}}>
        ...
        <FRow C={C} label="Tipo de Cobrança">
          <select value={p.tipo||"recorrente"} onChange={e=>updVS(i,"tipo",e.target.value)} style={iS(C)}>
            <option value="recorrente">Recorrente</option>
            <option value="parcelado">Parcelado</option>
          </select>
        </FRow>
        {p.tipo==="parcelado"
          ?<FRow C={C} label="Nº Parcelas"><Inp C={C} type="number" value={p.parcelas} onChange={v=>updVS(i,"parcelas",v)} placeholder="12" min="1"/></FRow>
          :<div style={{fontSize:11,color:C.muted,paddingBottom:4,alignSelf:"center"}}>Gera 12 meses</div>
        }
        {form.valueSchedule.length>1&&<button onClick={()=>ff({valueSchedule:form.valueSchedule.filter((_,j)=>j!==i)})} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:16,paddingBottom:4,alignSelf:"center",flexShrink:0}}>✕</button>}
      </div>)}
    </div>
...
function Funil({kanban,setKanban,stages,setStages,C}){
  ...
  function syncKanban(ns){setKanban(prev=>{const n={};ns.forEach(s=>{n[s]=prev[s]||[];});return n;});}
  function handleSetStages(fn){setStages(prev=>{const ns=typeof fn==="function"?fn(prev):fn;syncKanban(ns);return ns;});}
  function drop(e,col){
    if(!dragging||dragging.col===col)return;
    if(dragging.col!=="Fechado"&&col==="Fechado") setConfetti(true);
    setKanban(p=>{const n={...p};n[dragging.col]=(n[dragging.col]||[]).filter(c=>c.id!==dragging.card.id);n[col]=[...(n[col]||[]),dragging.card];return n;});
  ...
  return <div>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
      <div><h1 style={{fontFamily:"Syne",fontSize:26,fontWeight:800,color:C.strong}}>Funil de Vendas</h1>
        <p style={{color:C.muted,fontSize:13,marginTop:3}}>Pipeline total: <span style={{color:C.accent,fontWeight:700}}>R$ {total.toLocaleString()}</span></p></div>
      ...
      {stages.map(stage=><div key={stage} onDragOver={e=>{e.preventDefault();setOver(stage);}} onDrop={e=>drop(e,stage)}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:7,minHeight:50,maxHeight:360,overflowY:"auto"}}>
          {(kanban[stage]||[]).map(card=><div key={card.id} draggable onDragStart={()=>setDragging({card,col:stage})}
            ...
            <button onClick={e=>{e.stopPropagation();setKanban(p=>({...p,[stage]:(p[stage]||[]).filter(c=>c.id!==card.id)}));}} style={{position:"absolute",top:5,right:5,background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:11,opacity:.4}}>✕</button>
    ...
    {modal&&<Modal C={C} title="Nova Oportunidade" onClose={()=>setModal(false)}>
      ...
      <MFoot C={C} onClose={()=>setModal(false)} onSave={()=>{if(!form.name)return;setKanban(p=>({...p,[form.stage]:[...(p[form.stage]||[]),{id:uid(),name:form.name,value:Number(form.value)||0,resp:form.resp}]}));setModal(false);}} disabled={!form.name}/>
    </Modal>}
    {stageEd&&<ListEditor C={C} title="Etapas do Funil" items={stages} setItems={handleSetStages} onClose={()=>setStageEd(false)}/>}
  </div>;
}
...
function Atendimentos({history,setHistory,clients,attTypes,setAttTypes,C}){
  ...
  const [form,setForm]=useState({client:"",type:"",user:"",note:"",date:today(),time:""});
  ...
  function add(){
    ...
    setHistory(p=>[{...form,id:uid(),type:form.type||attTypes[0]},...p]);
    setModal(false);setForm({client:"",type:attTypes[0]||"",user:"",note:"",date:today(),time:""});
  }
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
      ...
      <div style={{display:"flex",gap:8}}><Btn C={C} ghost sm onClick={()=>setTypeEd(true)}>✏️ Tipos</Btn><Btn C={C} onClick={()=>{setForm({client:"",type:attTypes[0]||"",user:"",note:"",date:today(),time:""});setModal(true);}}>+ Registrar</Btn></div>
    </div>
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      {history.map(h=><div key={h.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"14px 18px",display:"flex",gap:14,boxShadow:C.shadow,position:"relative"}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{display:"flex",gap:6,marginBottom:8}}>
            ...
            <span style={{background:C.border,color:C.muted,padding:"2px 9px",borderRadius:20,fontSize:11}}>por {h.user}</span>
          </div>
          <p style={{fontSize:13,color:C.muted,lineHeight:1.6}}>{h.note}</p>
        </div>
        <button onClick={()=>setHistory(p=>p.filter(x=>x.id!==h.id))} style={{position:"absolute",top:10,right:12,background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:13,opacity:.4}}>✕</button>
      </div>)}
    </div>
    {modal&&<Modal C={C} title="Registrar Atendimento" onClose={()=>setModal(false)} wide>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        ...
        <FRow C={C} label="Responsável"><Inp C={C} value={form.user} onChange={v=>setForm(p=>({...p,user:v}))} placeholder="Nome"/></FRow>
    ...
    {typeEd&&<ListEditor C={C} title="Tipos de Atendimento" items={attTypes} setItems={setAttTypes} onClose={()=>setTypeEd(false)}/>}
  </div>;
}
...
function Tarefas({tasks,setTasks,clients,C}){
  ...
  const toggle=id=>setTasks(l=>l.map(t=>t.id===id?{...t,done:!t.done}:t));
  function add(){
    if(!form.title)return;
    if(form.due&&form.due<today()){setAlert("O prazo não pode ser inferior à data atual.");return;}
    setTasks(p=>[...p,{...form,id:uid(),done:false}]);
  ...
  const renderT=t=>{
    ...
    return <div key={t.id} style={{background:C.card,border:`1px solid ${late?C.danger:C.border}`,borderRadius:14,padding:"12px 16px",display:"flex",alignItems:"center",gap:12,opacity:t.done?.45:1,boxShadow:C.shadow}}>
      <div onClick={()=>toggle(t.id)} style={{width:19,height:19,borderRadius:5,border:`2px solid ${t.done?C.accent:C.border}`,background:t.done?C.accent:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{t.done&&<span style={{fontSize:10,color:"#fff",fontWeight:900}}>✓</span>}</div>
      ...
      <button onClick={()=>setTasks(l=>l.filter(x=>x.id!==t.id))} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:12,opacity:.4}}>✕</button>
    </div>;
  };
...
function Financeiro({rec,setRec,pay,setPay,clients,C}){
  ...
  useEffect(()=>{
    const t=today();
    setRec(p=>p.map(f=>f.status==="Pendente"&&f.due&&f.due<t?{...f,status:"Atrasado"}:f));
    setPay(p=>p.map(f=>f.status==="Pendente"&&f.due&&f.due<t?{...f,status:"Atrasado"}:f));
  },[]);
  ...
  function openEditRec(f){setEditItem({...f,_tipo:"rec"});}
  function openEditPay(f){setEditItem({...f,_tipo:"pay"});}
  function saveEdit(){
    if(editItem._tipo==="rec") setRec(p=>p.map(f=>f.id===editItem.id?{...editItem,value:Number(editItem.value)}:f));
    else setPay(p=>p.map(f=>f.id===editItem.id?{...editItem,value:Number(editItem.value)}:f));
    setEditItem(null);
  }
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:14,flexWrap:"wrap",marginBottom:18}}>
      {tab==="receber"?<>
        <Stat C={C} label="Recebido" value={`R$ ${paidRec.toLocaleString()}`} color={C.success}/>
        <Stat C={C} label="Atrasado" value={`R$ ${lateRec.toLocaleString()}`} color={C.danger}/>
        <Stat C={C} label="Total Esperado" value={`R$ ${rec.filter(f=>f.status!=="Cancelado").reduce((a,f)=>a+Number(f.value),0).toLocaleString()}`} color={C.accent}/>
      </>:<>
        <Stat C={C} label="Pago" value={`R$ ${paidPay.toLocaleString()}`} color={C.success}/>
        <Stat C={C} label="Pendente" value={`R$ ${pendPay.toLocaleString()}`} color={C.warning}/>
      </>}
    </div>
    ...
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,overflow:"hidden",boxShadow:C.shadow}}>
      <table style={{width:"100%",borderCollapse:"collapse"}}>
        <thead><tr style={{background:C.bg}}>
          {(tab==="receber"
            ?["Cliente","Descrição","Valor","Vencimento","Pagamento","Status","Ações"]
            :["Descrição","Fornecedor","Valor","Vencimento","Pagamento","Status","Ações"]
        ...
        <tbody>
          {tab==="receber"&&sortedAndFiltered(rec).map((f,i)=><tr key={f.id} style={{borderTop:`1px solid ${C.border}`,background:i%2===0?"transparent":C.stripe}}>
            ...
            <td style={{padding:"11px 14px"}}><div style={{display:"flex",gap:5,alignItems:"center"}}>
              {f.status!=="Pago"&&<Btn C={C} sm onClick={()=>{setBaixaDate(today());setBaixaModal({id:f.id,tipo:"rec"});}}>Baixar</Btn>}
              <Btn C={C} sm ghost onClick={()=>openEditRec(f)}>✏️</Btn>
          ...
          {tab==="pagar"&&sortedAndFiltered(pay).map((f,i)=><tr key={f.id} style={{borderTop:`1px solid ${C.border}`,background:i%2===0?"transparent":C.stripe}}>
            ...
            <td style={{padding:"11px 14px"}}><span style={{background:sColor(f.status)+"20",color:sColor(f.status),padding:"2px 9px",borderRadius:20,fontSize:11,fontWeight:700}}>{f.status}</span></td>
            <td style={{padding:"11px 14px"}}><div style={{display:"flex",gap:5,alignItems:"center"}}>
              {f.status!=="Pago"&&<Btn C={C} sm onClick={()=>{setBaixaDate(today());setBaixaModal({id:f.id,tipo:"pay"});}}>Baixar</Btn>}
              <Btn C={C} sm ghost onClick={()=>openEditPay(f)}>✏️</Btn>
              <button onClick={()=>setConfirm({id:f.id,tipo:"pay"})} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:13,opacity:.7}}>🗑</button>
            </div></td>
          </tr>)}
        </tbody>
      </table>
    ...
    {confirm&&<Confirm C={C} msg="Deseja realmente excluir este lançamento?" onNo={()=>setConfirm(null)} onYes={()=>{if(confirm.tipo==="rec") setRec(p=>p.filter(x=>x.id!==confirm.id)); else setPay(p=>p.filter(x=>x.id!==confirm.id));setConfirm(null);}}/>}

    {baixaModal&&<Modal C={C} title="Confirmar Baixa" onClose={()=>setBaixaModal(null)}>
      <p style={{color:C.text,fontSize:13,marginBottom:14}}>Informe a data de pagamento para confirmar a baixa:</p>
      <FRow C={C} label="Data da Baixa"><Inp C={C} type="date" value={baixaDate} onChange={v=>setBaixaDate(v)}/></FRow>
      <MFoot C={C} onClose={()=>setBaixaModal(null)} onSave={confirmBaixa} label="Confirmar Baixa" disabled={!baixaDate}/>
    </Modal>}
    ...
    {editItem&&<Modal C={C} title="Editar Lançamento" onClose={()=>setEditItem(null)} wide>
      {editItem._tipo==="rec"?<>
        ...
        <FRow C={C} label="Descrição"><Inp C={C} value={editItem.description} onChange={v=>setEditItem(p=>({...p,description:v}))} placeholder="Ex: Mensalidade Mar/25"/></FRow>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
    ...
    {modal==="rec"&&<Modal C={C} title="Nova Conta a Receber" onClose={()=>setModal(null)} wide>
      ...
      <MFoot C={C} onClose={()=>setModal(null)} onSave={()=>{if(!formR.client||!formR.value)return;setRec(p=>[...p,{...formR,id:uid(),value:Number(formR.value),status:"Pendente",paid:""}]);setModal(null);setFormR({client:"",value:"",due:"",description:""});}} disabled={!formR.client||!formR.value}/>
    </Modal>}
    ...
    {modal==="pay"&&<Modal C={C} title="Nova Conta a Pagar" onClose={()=>setModal(null)} wide>
      ...
      <MFoot C={C} onClose={()=>setModal(null)} onSave={()=>{if(!formP.description||!formP.value)return;setPay(p=>[...p,{...formP,id:uid(),value:Number(formP.value),status:"Pendente",paid:""}]);setModal(null);setFormP({description:"",supplier:"",value:"",category:"",due:""});}} disabled={!formP.description||!formP.value}/>
    </Modal>}
  </div>;
}
...
function Usuarios({users,setUsers,C}){
  ...
  function save(){
    ...
    if(modal==="new") setUsers(p=>[...p,obj]); else setUsers(p=>p.map(u=>u.id===obj.id?obj:u));
    setModal(null);
  }
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
    ...
    {confirm&&<Confirm C={C} msg="Deseja realmente excluir este usuário?" onNo={()=>setConfirm(null)} onYes={()=>{setUsers(p=>p.filter(x=>x.id!==confirm));setConfirm(null);}}/>}
    {modal&&<Modal C={C} title={modal==="new"?"Novo Usuário":"Editar Usuário"} onClose={()=>setModal(null)} wide>
      ...
      <FRow C={C} label="Senha">
        <div style={{position:"relative"}}><input value={form.password||""} onChange={e=>ff({password:e.target.value})} type={showPass?"text":"password"} placeholder="••••••••" style={iS(C)}/><button onClick={()=>setShowPass(s=>!s)} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:13}}>{showPass?"🙈":"👁"}</button></div>
      </FRow>
      <FRow C={C} label="Permissões de Acesso">
        <div style={{display:"flex",flexWrap:"wrap",gap:7,marginTop:5}}>
          {ALL_PAGES.map(page=>{const on=(form.permissions||[]).includes(page);return <button key={page} onClick={()=>togglePerm(page)} style={{padding:"5px 12px",borderRadius:20,border:`1px solid ${on?C.accent:C.border}`,background:on?C.accentDim:"transparent",color:on?C.accent:C.muted,cursor:"pointer",fontSize:12,fontWeight:on?700:400,transition:"all .12s"}}>{page}</button>;})}
        </div>
      </FRow>
...
export default function App(){
  const [theme,setTheme]=useState(()=>db.load("nd:theme","dark"));
  const [loggedIn,setLoggedIn]=useState(()=>db.load("nd:session",null));
  const [active,setActive]=useState("Dashboard");
  const [clients,setClients]=useState(()=>db.load("nd:clients",D_CLIENTS));
  const [kanban,setKanban]=useState(()=>db.load("nd:kanban",D_KANBAN));
  const [stages,setStages]=useState(()=>db.load("nd:stages",D_STAGES));
  const [history,setHistory]=useState(()=>db.load("nd:history",D_HISTORY));
  const [tasks,setTasks]=useState(()=>db.load("nd:tasks",D_TASKS));
  const [rec,setRec]=useState(()=>db.load("nd:rec",D_REC));
  const [pay,setPay]=useState(()=>db.load("nd:pay",D_PAY));
  const [users,setUsers]=useState(()=>db.load("nd:users",D_USERS));
  const [plans,setPlans]=useState(()=>db.load("nd:plans",D_PLANS));
  const [attTypes,setAttTypes]=useState(()=>db.load("nd:attTypes",D_ATT));
  const C=TH[theme];
  const toggleTheme=()=>setTheme(t=>t==="dark"?"light":"dark");

  useEffect(()=>db.save("nd:clients",clients),[clients]);
  useEffect(()=>db.save("nd:kanban",kanban),[kanban]);
  useEffect(()=>db.save("nd:stages",stages),[stages]);
  useEffect(()=>db.save("nd:history",history),[history]);
  useEffect(()=>db.save("nd:tasks",tasks),[tasks]);
  useEffect(()=>db.save("nd:rec",rec),[rec]);
  useEffect(()=>db.save("nd:pay",pay),[pay]);
  useEffect(()=>db.save("nd:users",users),[users]);
  useEffect(()=>db.save("nd:plans",plans),[plans]);
  useEffect(()=>db.save("nd:attTypes",attTypes),[attTypes]);
  useEffect(()=>db.save("nd:theme",theme),[theme]);
  useEffect(()=>db.save("nd:session",loggedIn),[loggedIn]);

  if(!loggedIn) return <Login users={users} onLogin={u=>setLoggedIn(u)} theme={theme} toggleTheme={toggleTheme}/>;

  const perms=loggedIn.permissions||ALL_PAGES;
  ...
  return <>
    ...
    `}</style>
    <div style={{display:"flex",height:"100vh",width:"100vw",overflow:"hidden",fontFamily:"DM Sans,sans-serif",background:C.bg,color:C.text}}>
      ...
      <div style={{width:220,minWidth:220,background:C.sidebar,borderRight:`1px solid ${C.sidebarBorder}`,display:"flex",flexDirection:"column",height:"100vh"}}>
        <div style={{padding:"20px 18px 16px",borderBottom:`1px solid ${C.sidebarBorder}`}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              <div style={{fontFamily:"Syne",fontWeight:800,fontSize:14,color:"#f1f5f9",lineHeight:1.2}}>Natividade</div>
              <div style={{fontFamily:"Syne",fontWeight:700,fontSize:9,color:"#6c63ff",letterSpacing:2}}>DIGITAL</div>
            </div>
          </div>
        </div>
        ...
        <div style={{padding:"12px 10px",borderTop:`1px solid ${C.sidebarBorder}`}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"6px 8px"}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <div style={{width:28,height:28,borderRadius:7,background:"#6c63ff22",display:"flex",alignItems:"center",justifyContent:"center",color:"#a8a3ff",fontWeight:800,fontFamily:"Syne",fontSize:12,flexShrink:0}}>{loggedIn.name[0]}</div>
              <div><div style={{fontSize:12,fontWeight:600,color:"#c8d0de"}}>{loggedIn.name}</div><div style={{fontSize:10,color:"#3d4f63"}}>{loggedIn.role}</div></div>
            </div>
            <button onClick={()=>setLoggedIn(null)} title="Sair" style={{background:"none",border:"none",color:"#4b5a72",cursor:"pointer",fontSize:14}}>⏏</button>
          </div>
        </div>
      </div>
